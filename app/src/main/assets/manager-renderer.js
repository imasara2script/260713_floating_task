...
function render() {
    ...
    if (managerList) {
        let html = '';
        if (isEditMode) {
            // 並べ替えモード時はグルーピングを解除して表示
            html = managerTasks.map((t) => {
            ...
            groups.forEach((key, groupIdx) => {
                ...
                html += tasksInGroup.map((t) => {
                    ...
                    <li class="task-item ${isTimer && !t.completed ? 'timer-active' : ''}" data-id="${t.id}">
                        ...
                        ${isTimer && !isExportMode && !isDeleteMode ? `<button class="btn-icon" onclick="repeatTimer(${t.id})" title="リピート">🔄</button>` : ''}
                    </li>
                    `;
                }).join('');
            });
